...
# West Build (left)
west build -s zmk/app -p -d build/left -b adv360pro_left -S studio-rpc-usb-uart -- -DZMK_CONFIG="${PWD}/config" -DCONFIG_ZMK_STUDIO=y
...
if [ "${BUILD_RIGHT}" = true ]; then
    # West Build (right)
    west build -s zmk/app -p -d build/right -b adv360pro_right -- -DZMK_CONFIG="${PWD}/config"
